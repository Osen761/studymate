...
function Install-WithWinget {
    ...
    if (-not (Test-CommandExists "winget")) {
        Write-Error "$DisplayName is missing and winget is not available. Install $DisplayName manually, reopen PowerShell, then rerun .\scripts\setup.ps1."
    }

    Write-Host "Installing $DisplayName with winget..."
    winget install --id $PackageId -e
    Write-Host ""
    Write-Host "$DisplayName was installed. Close and reopen PowerShell so PATH updates are loaded, then rerun .\scripts\setup.ps1."
    exit 0
}
...
Write-Host ""
Write-Host "StudyMate setup complete."
Write-Host "Start both servers with: .\scripts\start.ps1"
